...
if __name__ == '__main__':
    ...
    for ep in range(args.max_epoches):
        ...
        for iter, pack in enumerate(train_data_loader):
            ...
            l=avg_meter.get('loss')
            bar.set_description(f"epoch:{ep}")
            bar.set_postfix({'loss':'{:.5f}'.format(l),'lr': '{:.4f}'.format(optimizer.param_groups[0]['lr'])}
                      #'%.4f %.4f %.4f %.4f' % ,
            ...
            bar.update(1)
               ...
               # print('\rIter:%5d/%5d' % (optimizer.global_step-1, max_step),
